...
struct ContentView: View {
    ...
    @AppStorage("AIImageParseKey") private var aiParseKey: String = ""
    @AppStorage("AIImageParseURL") private var aiParseURL: String = ""
    @AppStorage("AIImageParseModel") private var aiParseModel: String = ""
    @AppStorage("AIImageParseFallbackModel") private var aiParseFallbackModel: String = ""
    @State private var showingCollabSheet = false
    @State private var tempAIKey: String = ""
    @State private var tempAIURL: String = ""
    @State private var tempAIModel: String = ""
    @State private var tempAIFallbackModel: String = ""
    ...
    var body: some View {
        NavigationView {
            ...
            .sheet(isPresented: $showingCollabSheet) {
                NavigationView {
                    Form {
                        Section(header: Text("AI 解析設定")) {
                            TextField("OpenRouter API Key", text: $tempAIKey)
                                .textInputAutocapitalization(.never)
                                .autocorrectionDisabled()
                            TextField("API URL（選填）", text: $tempAIURL)
                                .textInputAutocapitalization(.never)
                                .autocorrectionDisabled()
                            TextField("模型（選填）", text: $tempAIModel)
                                .textInputAutocapitalization(.never)
                                .autocorrectionDisabled()
                            TextField("備援模型（選填）", text: $tempAIFallbackModel)
                                .textInputAutocapitalization(.never)
                                .autocorrectionDisabled()
                        }
                    }
                    ...
                    .toolbar {
                        ToolbarItem(placement: .navigationBarLeading) {
                            Button("取消") { showingCollabSheet = false }
                        }
                        ToolbarItem(placement: .navigationBarTrailing) {
                            Button("儲存") {
                                aiParseKey = tempAIKey.trimmingCharacters(in: .whitespacesAndNewlines)
                                aiParseURL = tempAIURL.trimmingCharacters(in: .whitespacesAndNewlines)
                                aiParseModel = tempAIModel.trimmingCharacters(in: .whitespacesAndNewlines)
                                aiParseFallbackModel = tempAIFallbackModel.trimmingCharacters(in: .whitespacesAndNewlines)
                                showingCollabSheet = false
                            }
                        }
                    }
                    .onAppear {
                        tempAIKey = aiParseKey
                        tempAIURL = aiParseURL
                        tempAIModel = aiParseModel
                        tempAIFallbackModel = aiParseFallbackModel
                    }
                }
            }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 10) {
                    ...
                    Button(action: { showingCollabSheet = true }) {
